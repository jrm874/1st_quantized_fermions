...
class Hubbard(CustomHilbert):
    r"""Hilbert space obtained as tensor product of local spin states for the Hubbard model"""
    ...
    def __init__(
        ...
        Examples:
           ...
           >>> from netket.hilbert import Hubbard
    ...
    def _random_state_with_constraint_non_repeating_clock_hubbard(self, out, rgen):
        sites = list(range(self.size))
        out.fill(-round(2 * self._s))
        Num_fermions = int(2 * self._s)
        list_positions_up = _np.random.choice(_np.arange(int(self.size/2.)), size = Num_fermions, replace = False).astype('int')
        list_positions_down = _np.random.choice(_np.arange(int(self.size/2.)), size = Num_fermions, replace = False).astype('int') + int(self.size/2.)

        for i in range(Num_fermions):
            out[list_positions_up[i]] = round(2 * (-self._s + 1 + i))

        for i in range(Num_fermions):
            out[list_positions_down[i]] = round(2 * (-self._s + 1 + i))
        ...
        if self._total_sz is None:
            ...
            if size is not None:
                ...
                for b in range(out_r.shape[0]):
                    self._random_state_with_constraint_non_repeating_clock_hubbard(out_r[b], rgen)
            else:
                self._random_state_with_constraint_non_repeating_clock_hubbard(out, rgen)
    ...
    def __pow__(self, n):
        ...
        return Hubbard(self._s, self.size * n, total_sz=total_sz)
